...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST["idlecturer"])) {
        $idlecturer = $_POST["idlecturer"];

        $stmt_check = $conn->prepare("SELECT idlecturer FROM conferencistas WHERE idlecturer = ?");
        $stmt_check->bind_param("i", $idlecturer);
        ...
        if ($result->num_rows > 0) {
            $stmt_delete = $conn->prepare("DELETE FROM `conferencistas` WHERE idlecturer = ?");
            $stmt_delete->bind_param("i", $idlecturer);
